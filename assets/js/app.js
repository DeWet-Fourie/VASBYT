...
function placeholder(label = 'V') {
  return `<div class="placeholder-art" aria-label="Image placeholder">${escapeHtml(label[0] || 'V')}</div>`;
}
...
function renderNav() {
  ...
  const collectionLinks = collections.map(c => `<a href="shop.html?collection=${encodeURIComponent(c.id)}">${escapeHtml(c.navLabel || c.label)}</a>`).join('');
  ...
  return `
    <article class="product-card">
      <a class="product-media" href="${productUrl(product)}">
        ${image ? `<img src="${image}" alt="${escapeHtml(product.name)}" loading="lazy">` : placeholder(product.name)}
...
function renderProductDetail() {
  ...
  mount.innerHTML = `
    <div class="product-detail-media">${image ? `<img src="${image}" alt="${escapeHtml(p.name)}">` : placeholder(p.name)}</div>
