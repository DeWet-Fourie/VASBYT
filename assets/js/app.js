...
function openQuote(){
  ...
  const items = state.quote.map(item => { const p = productById(item.id); if(!p) return ''; return `<div class="quote-item"><span><b>${esc(p.name)}</b><small>${money(p.salePrice||p.price)} each</small></span><div class="quote-controls"><button data-dec="${esc(item.id)}" aria-label="Decrease quantity">−</button><b>${item.qty}</b><button data-inc="${esc(item.id)}" aria-label="Increase quantity">+</button><button data-remove="${esc(item.id)}" aria-label="Remove item">×</button></div></div>`; }).join('');
